...
export class SpecialistSearchService {
  ...
  async searchSpecialists(filters: SearchFilters): Promise<SearchResults> {
    try {
      ...
      // Calculate facets for future use (simplified for now)
      const facets = {
        skills: this.calculateSkillFacets(specialists),
        specializations: this.calculateSpecializationFacets(specialists),
      }
      ...
      return this.searchSpecialistsFake(filters)
    }
  }

  private async searchSpecialistsFake(filters: SearchFilters): Promise<SearchResults> {
    // Simulate API delay
